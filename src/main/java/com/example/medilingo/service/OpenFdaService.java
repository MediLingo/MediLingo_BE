...
public class OpenFdaService {
    ...
    private List<DrugEntry> queryLabel(String activeIngredient, boolean otcOnly, int limit) {
        try {
            ...
            if (otcOnly) search += " AND openfda.product_type:\"OTC\"";
            ...
            if (raw == null || raw.isBlank()) {
                log.warn("OpenFDA returned empty for '{}' (otcOnly={})", activeIngredient, otcOnly);
                return List.of();
            }
            ...
            log.info("No OpenFDA results for '{}' (otcOnly={})", activeIngredient, otcOnly);
            return List.of();
        } catch (WebClientResponseException.TooManyRequests e) {
            log.warn("OpenFDA rate limit exceeded");
            return List.of();
        } catch (Exception e) {
            log.error("OpenFDA call failed for '{}': {}", activeIngredient, e.getMessage());
            return List.of();
        }
    }
    ...
    private List<DrugEntry> parseEntries(String json) {
        try {
            ...
            for (JsonNode result : results) {
                ...
                String primaryName = (brandName != null && !brandName.isBlank())
                ...
                // NEW — extract ALL substance names instead of just the first
                List<String> allIngredients = allArrayValues(openfdaNode, "substance_name")
                    .stream()
                    .map(String::toLowerCase)
                    .toList();

                String displayName = buildDisplayName(
                    primaryName, parsedStrength, dosageForm, manufacturer);
                if (displayName == null) continue;

                String nameForImage = (brandName != null && !brandName.isBlank())
                    ? brandName : genericName;

                entries.add(new DrugEntry(nameForImage, splSetId, displayName, allIngredients));
            }
    ...
    // NEW helper — returns all values from an array field
    private List<String> allArrayValues(JsonNode node, String fieldName) {
    ...
    private String parseStrengthFromText(String text) {
        ...
        java.util.regex.Matcher m = java.util.regex.Pattern
            .compile("(\\d+(?:\\.\\d+)?\\s*(?:mg|mcg|g|mL|%|IU))",
                java.util.regex.Pattern.CASE_INSENSITIVE)
            .matcher(text);
        return m.find() ? m.group(1).trim() : null;
    }

    private String buildDisplayName(String name, String strength,
        String dosageForm, String manufacturer) {
